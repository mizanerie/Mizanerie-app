...
/** Sur un appareil partagé, la session n'est pas conservée à la fermeture. */
let persist = true;
export function setPersist(v) {
  persist = !!v;
  if (!persist) localStore.del(SESSION_KEY);
  else if (session) localStore.set(SESSION_KEY, JSON.stringify(session));
}

function setSession(s) {
  session = s;
  if (s && persist) localStore.set(SESSION_KEY, JSON.stringify(s));
...
const ingToRow = (i) => ({
  ...
  stock: i.stock === "" || i.stock === undefined || i.stock === null ? null : num(i.stock),
  stock_min: i.stockMin === "" || i.stockMin === undefined || i.stockMin === null ? null : num(i.stockMin),
});
const rowToIng = (r) => ({
  ...
  stock: r.stock === null || r.stock === undefined ? null : str(r.stock),
  stockMin: r.stock_min === null || r.stock_min === undefined ? null : str(r.stock_min),
...
/* ─────────── Stock ─────────── */
export const applyStockMoves = (moves, reason = "production", prod = null) =>
  db("/rpc/apply_stock_moves", { method: "POST", body: { moves, reason, prod } });
export const stockMoves = (ingredientId) =>
  db(`/rest/v1/stock_moves?select=*&ingredient_id=eq.${ingredientId}&order=created_at.desc&limit=30`.replace("/rest/v1", ""), {});

export const deleteAccount = () => db("/rpc/delete_my_account", { method: "POST", raw: true });


/* ─────────── Administration (réservée aux administrateurs) ─────────── */
export const isAdmin = () => db("/rpc/is_admin", { method: "POST", body: {} });
export const adminListUsers = () => db("/rpc/admin_list_users", { method: "POST", body: {} });
export const adminSetPlan = (target, plan, months = null) =>
  db("/rpc/admin_set_plan", { method: "POST", raw: true, body: { target, new_plan: plan, months } });
