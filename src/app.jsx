...
const emptyData = () => ({
  version: 1,
  settings: { businessName: "", currency: "MAD", lang: detectLang(), priceLabel: "aucun", marginMode: "prix_vente", defaultRatePct: "", onboarded: false },
  ...
  return (
    <div className="welcome">
      <div className="welcome-card">
        ...
        <h1>{t("w.title")}</h1>
        <p>{t("w.text")}</p>
        <Field label={t("set.language")}><LangSelect value={lang} onChange={onLang} /></Field>
  ...
  if (!data.settings.onboarded) {
    return (
      <div className="app" dir={dir} lang={LANG}><style>{CSS}</style>
        <Welcome lang={data.settings.lang} onLang={(l) => update((p) => ({ ...p, settings: { ...p.settings, lang: l } }))}
          onDone={(name, cur) => update((p) => ({ ...p, settings: { ...p.settings, businessName: name, currency: cur, onboarded: true } }))} />
      </div>
    );
  }
